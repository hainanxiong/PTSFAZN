%% PTCZN Baseline: Pose Tracking With Online Jacobian Estimation
% This script evaluates a PTCZN-based pose-tracking controller on a PUMA
% 560 manipulator. The task combines a multi-loop three-dimensional position
...
%   1. All trajectory, controller, neural-dynamics, projection, activation,
%      and Jacobian-estimation parameters are defined explicitly below.
%   2. PTCZN differs from the LZN baseline primarily through the nonlinear
%      activation applied to the position and orientation errors.
...
xi2 = 0.8;                 % Orientation-error feedback gain
zeta = 1;                  % Jacobian-estimation gain

% Nonlinear PTCZN activation parameters used in activation_function().
activation_k1 = 0.5;
activation_k2 = 0.5;
...
for k = 1:num_samples
    ...
    % Skew-symmetric orientation error used by the PTCZN controller.
    orientation_error_controller = 0.5 * vee3( ...
        current_rotation * desired_rotation_k' - ...
        desired_rotation_k * current_rotation');

    % Nonlinear activation used by the PTCZN baseline.
    activated_position_error = activation_function( ...
        position_error, activation_k1, activation_k2, activation_alpha);
    activated_orientation_error = activation_function( ...
        orientation_error_controller, ...
        activation_k1, activation_k2, activation_alpha);
...
PTCZN_P_rmse = sqrt(mean(ep_log.^2, 2));
PTCZN_O_rmse = sqrt(mean(eo_log.^2, 2));
PTCZN_P_mae = mean(abs(ep_log), 2);

fprintf('\n================= PTCZN Tracking Summary ==================\n');
fprintf('Position RMSE [x y z] (m):       %.6f  %.6f  %.6f\n', ...
    PTCZN_P_rmse(1), PTCZN_P_rmse(2), PTCZN_P_rmse(3));
fprintf('Orientation RMSE [x y z] (rad):  %.6f  %.6f  %.6f\n', ...
    PTCZN_O_rmse(1), PTCZN_O_rmse(2), PTCZN_O_rmse(3));
fprintf('Position MAE [x y z] (m):        %.6f  %.6f  %.6f\n', ...
    PTCZN_P_mae(1), PTCZN_P_mae(2), PTCZN_P_mae(3));
fprintf('=============================================================\n');
...
link_colors = {'r', 'c', [0.8, 0.8, 0.8], 'g', 'r', 'b'};
...
plot3(actual_position_log(1, :), actual_position_log(2, :), ...
    ...
    'Marker', 'v', 'MarkerIndices', 1:100:num_samples, ...
...
function output = activation_function(value, k1, k2, alpha)
%ACTIVATION_FUNCTION Nonlinear PTCZN activation used in the supplied code.
output = ( ...
    k1 * abs(value).^(1 - alpha/2) + ...
    k2 * abs(value).^(1 + alpha/2)) .* sign(value);
end
