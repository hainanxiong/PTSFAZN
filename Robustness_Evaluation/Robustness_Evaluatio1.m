%% Scheme 2: Hybrid Orientation and Position Control With D-H Perturbations
...
% This version retains the time-varying D-H-parameter perturbations from the
% supplied comparison script.
%
% Requirements:
%   1. MATLAB.
...
%   1. All controller, trajectory, integration, constraint, and perturbation
%      parameters used in this comparison are defined explicitly below.
%   2. The controller uses the geometric Jacobian returned by robot.jacob0
%      for the current perturbed robot model, preserving the supplied setup.
...
% Outer-loop feedback gains.
position_gain = 15;
orientation_gain = 1;

% DNN design parameters in Eqs. (22a) and (22b) of the reference method.
dnn_mu = 25;               % Eq. (22a)
dnn_nu = 15;               % Eq. (22b)

% Inner-loop discretization.
num_inner_steps = 40;
...
% vartheta: DNN internal state; the joint velocity is its box projection.
% y_aux:     auxiliary DNN state corresponding to y (or gamma) in the
...
for k = 1:num_samples
    ...
    % In Peter Corke's Robotics Toolbox, Link objects are reference objects;
    % therefore, these updates are shared with 'robot' and affect its
    % subsequent forward-kinematics and Jacobian evaluations.
    ...
    for inner_idx = 1:num_inner_steps
        ...
        % Eq. (22b): y_dot = nu * (v_d - J*J'*y).
        y_dot = dnn_nu * ( ...
            task_velocity_command - ...
            task_jacobian * task_jacobian' * y_aux);

        % Eq. (22a):
        % vartheta_dot = P_Lambda(mu*J'*(v_d-v) + J'*y).
        % The box projection below is used for P_Lambda under the imposed
        % joint-velocity bounds, consistent with the supplied script.
